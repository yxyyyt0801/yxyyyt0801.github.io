...
### Eureka server 间同步三级级队列

**用空间换时间，用异步换吞吐**。它把同步任务从“单条处理”变成了“打包处理”，极大降低了网络 I/O 开销。

#### 第一级：AcceptorQueue（接收缓冲区）

这是数据进入系统的入口。当服务端收到注册、续约或下线请求时，并不是立刻去同步其他节点，而是先将任务封装成 `TaskHolder`扔进这个队列。

- **核心作用**：**削峰填谷**。无论前端请求多么密集，这里只管接收，不阻塞主流程。
- **数据结构**：`LinkedBlockingQueue`（无界阻塞队列）。



#### 第二级：ProcessingOrder（排序与去重）

这是整个机制的大脑，由一个后台线程 `AcceptorRunner`负责搬运和整理。它主要解决两个问题：

1. **合并任务**：如果同一个服务实例（ID相同）在短时间内发了多次心跳或更新，`ProcessingOrder`会将其合并，只保留最新的一次，避免无效同步。
2. **确定优先级**：它维护了一个双端队列（Deque）。 **新任务**：从队尾进入（FIFO）。 **重试任务**：从队头插入（优先处理），确保之前同步失败的任务能尽快重试。
   - 重试任务：“Eureka Server 在同步注册信息给其他节点时，如果某次同步失败（比如网络抖动），会生成一个‘重试任务’。这个任务会**插入到 ProcessingOrder 队列的头部**，而不是尾部。



#### 第三级：BatchWorkQueue（打包发车）

这是执行前的最后一步。==当 `ProcessingOrder`中的任务积累到一定数量（默认 250 个）或者等待时间超过阈值（默认 500ms）时，系统会把这些任务打包成一个 `List`，扔进 `BatchWorkQueue`==。

- **核心作用**：**批量提交**。工作线程（Worker）从这里拿走一整包任务，通过**一次 HTTP 请求**批量发送给其他 Eureka 节点。
- **收益**：将原本可能需要发送 250 次的网络请求，压缩成了 1 次，大幅提升了同步效率。
